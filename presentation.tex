    ...
    \begin{tabular}{cc}
      \begin{minipage}{.5\linewidth}
        ...
        \begin{tikzpicture}[font=\sffamily\tiny]
          ...
          \path[->] (P1) edge  node[left] {$coin$} (P2);
          \path[->] (P1) edge  node[right] {$coin$} (P3);
          \path[->] (P2) edge  node[left] {$get-tea$} (P4);
          \path[->] (P3) edge  node[right] {$get-coffee$} (P5);
          \path[->] (P4) edge [bend right=25]  node[left] {$tea$} (P1);
          \path[->] (P5) edge [bend left=25]  node[right] {$coffee$} (P1);
        \end{tikzpicture}
      \end{minipage}
      \begin{minipage}{.5\linewidth}
        \centering
        \begin{tikzpicture}[font=\sffamily\tiny]
          ...
          \path[->] (Q1) edge  node[left] {$coin$} (Q2);
          \path[->] (Q2) edge  node[right] {$get-coffee$} (Q3);
          \path[->] (Q2) edge   node[left] {$get-tea$} (Q4);
          \path[->] (Q3) edge  [bend right=40] node[right] {$coffee$} (Q1);
          \path[->] (Q4) edge   [bend left=40] node[left] {$tea$} (Q1);
        \end{tikzpicture}
      \end{minipage}
    \end{tabular}
  \end{frame}
